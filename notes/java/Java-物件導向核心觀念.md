...
在 Java 後端開發中，掌握物件導向 (OOP) 的核心機制與底層運作，是寫出高內聚、低耦合程式碼的基礎。這篇筆記主要記錄 Java 的存取修飾子、封裝、繼承、多型以及 Static 關鍵字的實戰應用。
...
- **靜態方法** (Static Methods)：
  ...
  - > ⚠️ 靜態方法內不能直接存取非靜態（實體）的變數或方法，也不能使用 `this` 或 `super`，因為靜態方法執行時，物件可能根本還沒被創建。
